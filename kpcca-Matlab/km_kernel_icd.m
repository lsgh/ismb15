function [G,subset] = km_kernel_icd(X,ktype,kpar,m,precision, w)
%% NOTE: this method has been modified by Laleh Soltan Ghoraie from
%% the original version to pass an extra parameter: weights (w) 
...
for i = 1:m,	% column index
	...
	if i == 1
		...
		d(i:n) = km_kernel(x,x,[ktype '-diag'],kpar, w, perm)';  
	else
		% update the diagonal of the residual kernel matrix
		d(i:n) = km_kernel(x,x,[ktype '-diag'],kpar, w, perm)' - sum(G(i:n,1:i-1).^2,2)';   
	end
	dtrace = sum(d(i:n));
	...
	G(i+1:n,i) = (km_kernel(X(perm(i),:),X(perm(i+1:n),:),ktype,kpar, w, perm)'...    
